...
public class UserLogin extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
...

        {
            ...
            request.getSession().setAttribute(ExamConstant.SESSION_LOGIN_ID,userID);
            if (ExamConstant.LOGIN_TYPE_VOLUNTEER.equals(type))//如果是用户类型
            {
                Volunteer model=VolunteerService.login(userID,password);
                if (model != null) {
                    loginFlag = true;
                ...
                Manager model = ManagerService.login(userID, password);
                if (model != null) {
                    loginFlag = true;
                    //得到管理人员对应的管理区域
